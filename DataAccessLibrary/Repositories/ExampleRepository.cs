...
namespace DataAccessLibrary.Repositories
{
    public class ExampleRepository : IExampleRepository
    {
        ...
        public async Task<IEnumerable<ExampleDTO>> GetAllExamplesAsync(int pageNumber, int pageSize, string? serverSearchTerm)
        {
            using var context = _contextFactory.CreateDbContext();
            List<Example> Examples;
            if (!string.IsNullOrWhiteSpace(serverSearchTerm))
            {
                Examples = await context.Examples
                                        .Where(v =>
                    (v.Text != null && v.Text.ToLower().Contains(serverSearchTerm))
                     || (v.LargeText != null && v.LargeText.ToLower().Contains(serverSearchTerm))
                    )
