...
{
    public static class RepositoryExtractorTester
    {
        public static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine($"\n{'=',50}");
                var result = TestExtractRepositoryFromPath(path);
                Console.WriteLine($"RESULT: {result ?? "NULL"}");
                Console.WriteLine($"{'=',50}");
            }
        }public static string? TestExtractRepositoryFromPath(string filePath)
        {
            try
            {
