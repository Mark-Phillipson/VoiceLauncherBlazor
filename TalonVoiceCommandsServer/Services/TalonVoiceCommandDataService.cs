...
public class TalonVoiceCommandDataService : ITalonVoiceCommandDataService
{
    ...
    private async Task LoadFromLocalStorageAsync()
    {
        try
        {
            ...
            var commandsJson = await _jsRuntime.InvokeAsync<string>("localStorage.getItem", CommandsStorageKey);
            if (!string.IsNullOrEmpty(commandsJson))
            {
                _commands = JsonSerializer.Deserialize<List<TalonVoiceCommand>>(commandsJson) ?? new List<TalonVoiceCommand>();
                _nextCommandId = _commands.Count > 0 ? _commands.Max(c => c.Id) + 1 : 1;
            ...
            var listsJson = await _jsRuntime.InvokeAsync<string>("localStorage.getItem", ListsStorageKey);
            if (!string.IsNullOrEmpty(listsJson))
            {
                _talonLists = JsonSerializer.Deserialize<List<TalonList>>(listsJson) ?? new List<TalonList>();
                _nextListId = _talonLists.Count > 0 ? _talonLists.Max(l => l.Id) + 1 : 1;
    ...
    private async Task SaveCommandsToLocalStorageAsync()
    {
        try
        {
            var commandsJson = JsonSerializer.Serialize(_commands);
            await _jsRuntime.InvokeVoidAsync("localStorage.setItem", CommandsStorageKey, commandsJson);
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Error saving commands to local storage: {ex.Message}");
        }
    }

    private async Task SaveListsToLocalStorageAsync()
    {
        try
        {
            var listsJson = JsonSerializer.Serialize(_talonLists);
            await _jsRuntime.InvokeVoidAsync("localStorage.setItem", ListsStorageKey, listsJson);
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Error saving lists to local storage: {ex.Message}");
        }
    }
    ...
    private int GenerateNextListId()
    {
        var id = _nextListId;
        _nextListId++;
        return id;
    ...
    public async Task<int> ImportTalonListsFromFileAsync(string filePath)
    {
        ...
        _talonLists.Clear();
        _talonLists.AddRange(talonLists);
        await SaveListsToLocalStorageAsync();
    ...
    public async Task<List<TalonVoiceCommand>> SemanticSearchWithListsAsync(string searchTerm)
    {
        ...
        if (string.IsNullOrWhiteSpace(searchTerm))
        {
            return _commands.OrderByDescending(c => c.CreatedAt).Take(100).ToList();
        }
        ...
        try
        {
            ...
            // Perform basic text search instead of semantic search
            var directMatches = allCommands
            ...
            // Combine direct matches and list-referencing commands, removing duplicates
            ...
            Console.WriteLine($"[DEBUG] Final results: {directMatches.Count} direct matches + {listReferencingCommands.Count} list-referencing commands = {finalResults.Count} total");
            return finalResults;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[DEBUG] Search failed, falling back to basic search: {ex.Message}");
    ...
    public async Task<List<TalonVoiceCommand>> SearchCommandNamesOnlyAsync(string searchTerm)
    {
        ...
        Console.WriteLine($"[DEBUG] Command names only search for '{searchTerm}': {results.Count} results");
        return results;
    }
    ...
    public async Task<List<TalonVoiceCommand>> SearchScriptOnlyAsync(string searchTerm)
    {
        ...
        var scriptMatches = _commands
            .Where(c => (c.Script ?? "").ToLower().Contains(lowerTerm))
            .OrderByDescending(c => c.CreatedAt)
            .Take(100)
            .ToList();

        Console.WriteLine($"[DEBUG] Script only search for '{searchTerm}': {scriptMatches.Count} results");
    ...
    /// <summary>
    /// Semantic search (simplified to basic text search for now)
    /// </summary>
    public async Task<List<TalonVoiceCommand>> SemanticSearchAsync(string searchTerm)
    {
        // For now, just use the comprehensive search
        return await SemanticSearchWithListsAsync(searchTerm);
    }
    ...
    public async Task<int> ImportTalonListsFromFileWithAutoFixAsync(string filePath)
    {
        ...
        // Auto-fix column sizes if needed (not needed for local storage)
        // var maxLengths = GetTalonListMaxLengths(talonLists);
        ...
        // Apply schema changes if any (not needed for local storage)
